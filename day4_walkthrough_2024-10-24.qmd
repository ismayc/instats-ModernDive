...
more than this hypothesized value of 4100 grams. We have a sample of 333 penguins from the `penguins` dataset after removing missing values.
...
Since the $p$-value = 0.012 is less than $\alpha = 0.05$, we reject the null
hypothesis. We, therefore, have enough evidence to suggest that the average
body mass of Palmer penguins is greater than the previously assumed value of
4100 grams.
...
# Get sample statistic (observed test statistic)
...
visualize(null_dist_t) +
  shade_p_value(obs_stat = obs_stat_t, direction = "right")
...
t.test(x = penguins_data$body_mass_g,
       mu = mu_hypothesized,
       alternative = "greater")
```
...
adelie_chinstrap_data <- penguins_data |> 
  ...
  # Not required but gets rid of annoying reminder
  droplevels()
...
Since $p$-value = 0.736 is greater than $\alpha = 0.1$, we fail to reject the
null hypothesis. We, therefore, DO NOT have enough evidence to suggest that the
aerage body mass of penguins is different for the Adelie group compared to the
Chinstrap group.
...
theory_p_value <- null_distro_t |> 
  get_p_value(obs_statistic_t, direction = "two sided")
```
...
t.test(formula = body_mass_g ~ species, 
       data = adelie_chinstrap_data,
       alternative = "two.sided")

# Also check out in the `infer`
?t_test
```
...
The value of 0 is in the interval, so it is a plausible value for the difference
of means in Adelie minus Chinstrap. So from the confidence interval, we also
fail to reject the null hypothesis. Remember that the confidence level + significance
level must be 100%.
...
# Plot a matrix to view the relationships between variables using GGally
ggpairs(data = model_data, mapping = aes(color = species))
```
...
# Looking to see if some type of pattern exists in the data of
# fitteds vs residuals
ggplot(fit_and_res_mult, aes(x = body_mass_g_hat, y = residual)) +
  geom_point()

# Visualize the residuals using a QQ plot

# Can also use a histogram to check
ggplot(fit_and_res_mult, aes(x = residual)) +
  geom_histogram()

# QQplot
ggplot(fit_and_res_mult, aes(sample = residual)) +
  geom_qq() +
  geom_qq_line(color = "blue", linewidth = 0.5)
```
