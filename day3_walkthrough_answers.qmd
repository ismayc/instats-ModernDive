...
We are "95% confident" that the true mean commute time for this population of adults is between `r bootstrap_percentile_ci$lower` and `r bootstrap_percentile_ci$upper` minutes. This is the same as saying that if we were to take many samples of size 100 and calculate the confidence interval for each sample, we would expect 95% of those intervals to contain the true population mean of `r mu`.
